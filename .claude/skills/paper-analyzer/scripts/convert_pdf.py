#!/usr/bin/env python3
"""
PDF conversion script – supports multiple conversion methods
1. Read output from the MinerU desktop application (recommended)
2. Use PyMuPDF with page screenshots (fallback)
"""
...
def find_mineru_output(pdf_name: str, mineru_dir: str = "~/MinerU") -> Optional[Path]:
    """Find the output directory generated by the MinerU desktop application"""
    mineru_path = Path(mineru_dir).expanduser()
    if not mineru_path.exists():
        return None

    # MinerU output directory format: filename.pdf-uuid
    for item in mineru_path.iterdir():
        if item.is_dir() and item.name.startswith(pdf_name):
            # Check whether full.md exists
...
def copy_mineru_output(mineru_dir: Path, output_dir: Path) -> Dict:
    """Copy MinerU output into the target directory"""
...
def convert_with_pymupdf(pdf_path: Path, output_dir: Path) -> Dict:
    """Convert using PyMuPDF (fallback option)"""
    try:
        import fitz
    except ImportError:
    ...
    for page_num, page in enumerate(doc, 1):
        ...
        # If there are no embedded images, capture the full page as an image
...
def main():
    if len(sys.argv) < 2:
        ...
        print("  2. Fall back to PyMuPDF if MinerU output is not found")
        sys.exit(1)
